<?php
	...
	if($checkbox == 'SINGLE'){
		...
		if($quiz_type == 'WORD' or $quiz_type =='VIDEO'){
			$sql_word = "select * FROM QuestionList WHERE No like '".$q_list[$sql_number]."' AND QA like 'A2'";
			$result = mysqli_fetch_object($db->query($sql_word));
			echo "<input type='button' onclick=reply_b() value='".$result->Content."'>";
		}
		else{
			$sql_picture = "select * from QuestionList where No like '".$q_list[$sql_number]."' and QA like 'A2'";//抓圖片相關資訊	
			$stmt = $db->query($sql_picture);
			$result = mysqli_fetch_object($stmt);
			$picture_type = $result->picture_ext;//抓圖片型態	
			$picture_alt = $result->picture_alt;//抓圖片alt
			
			echo "<input type='image' src='http://10.16.1.13/chen_IRS/upload/Q".$q_list[$sql_number]."A2.".$picture_type."' alt='".$picture_alt."' width='80%' height='80%' onclick='reply_b()'>";
		}
	}
	else{
		$sql_quiz_type = "select * from QuestionList where No like '".$q_list[$sql_number]."' and QA like 'Q'";
		...
		if($quiz_type == 'WORD' or $quiz_type =='VIDEO'){
			$sql_word = "select * FROM QuestionList WHERE No like '".$q_list[$sql_number]."' AND QA like 'A2'";
			$result = mysqli_fetch_object($db->query($sql_word));
			echo "<input type='checkbox' onclick=reply_b() value='".$result->Content."'>";
		}
		else{
			$sql_picture = "select * from QuestionList where No like '".$q_list[$sql_number]."' and QA like 'A2'";//抓圖片相關資訊
			$stmt = $db->query($sql_picture);
			...
			echo "<input type='checkbox' id='A2'/>";
			echo "<label for='A2'><img src='http://10.16.1.13/chen_IRS/upload/Q".$q_list[$sql_number]."A2.".$picture_type."' alt='".$picture_alt."' width='80%' height='80%' onclick='reply_b()'></label>";
		}
	}
