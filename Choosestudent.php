<?
	...
	$sql_teacherclass = "select * from ClassList where TeacherID = '".$teacher_id."'";
		...
		if($check_i!=$check_count){
			if($checkstudentnumberarray[$check_i] == $studentnumberarray[$i]){
				...
				echo "<input type='checkbox' name='value[]' id='".$studentnumberarray[$i]."' value='".$studentnumberarray[$i]."'  checked>";
					echo "<label style='word-wrap:break-word;' class='square-button rwdtxt' for='".$studentnumberarray[$i]."'>".$schoolarray[$i].$gradearray[$i].$classarray[$i].$idarray[$i].$namearray[$i]."</label>";
				echo "</td>";
				echo "</tdr>";
				$check_i++;
			}
